...
def saveDict(fn,dict_rap):
    f = open(fn,'wb')
    w = csv.writer(f)
    for key, val in list(dict_rap.items()):
...
def _getStrikeAscents(fontFileName, size):
    ...
    (wd_o, ht_o) = ttf.getsize('o')    # used as the baseline character
    
    (wd_b, ht_b) = ttf.getsize('b')    # representative of group 1
    (wd_t, ht_t) = ttf.getsize('t')    # representative of group 2
    (wd_A, ht_A) = ttf.getsize('A')    # representative of group 3
    (wd_0, ht_0) = ttf.getsize('0')    # representative of group 4
    (wd_ques, ht_ques) = ttf.getsize('?')  # representative of group 5
    (wd_and, ht_and) = ttf.getsize('&')    # representative of group 6
    
    (wd_sap, ht_sap) = ttf.getsize('à')   # representative of group 7
    (wd_cAp, ht_cAp) = ttf.getsize('À')   # representative of group 8 
    (wd_beta, ht_beta) = ttf.getsize('ß')  # representative of group 9
    
    return ({'bdfhijkl|()\"\'': ht_b - ht_o,
             't': ht_t - ht_o,
             'ABCDEFGHIJKLMNOPQRSTUVWXYZ': ht_A - ht_o,
             '0123456789': ht_0 - ht_o,
             '?!': ht_ques - ht_o,
             '%&@$': ht_and - ht_o,
             'àáâãäåèéêëìíîïñòóôõöùúûüāăćĉċčēĕėěĩīĭńňōŏőŕřśŝšũūŭůűŵźżž': ht_sap - ht_o,
             'ÀÁÂÃÄÅÆÈÉÊËÌÍÎÏÐÑÒÓÔÕÖØÙÚÛÜÝĀĂĆĈĊČĎĒđĔĖĚĜĞĠĤĥĴĹĺłŃŇŊŌŎŐŒŔŘŚŜŠŤŦŨŪŬŮŰŴŶŸŹŻŽƁſ': ht_cAp - ht_o,
             'ßÞƀƄƚɓɫɬʖʕʔʡʢ': ht_beta - ht_o             
            })
...
def _getStrikeDescents(fontFileName, size):
    ...
    """
    ttf = ImageFont.truetype(fontFileName, size) # init the specified font resource
    (wd_o, ht_o) = ttf.getsize('o')    # used as the baseline for "gpqy"    
    (wd_g, ht_g) = ttf.getsize('g')

    (wd_i, ht_i) = ttf.getsize('i')    # used as the baseline for "j"
    (wd_j, ht_j) = ttf.getsize('j')

    (wd_O, ht_O) = ttf.getsize('O')    # used as the baseline for "Q@&$"
    (wd_Q, ht_Q) = ttf.getsize('Q')

    (wd_dot, ht_dot) = ttf.getsize('.')    # used as the baseline for ","
    (wd_comma, ht_comma) = ttf.getsize(',')
    
    (wd_colon, ht_colon) = ttf.getsize(':') # used as the baseline for ";"        
    (wd_semicolon, ht_semicolon) = ttf.getsize(';')
    
    (wd_l, ht_l) = ttf.getsize('l')    # used as the baseline for "|()_"
    (wd_or, ht_or) = ttf.getsize('|')  
    
    (wd_scbelow, ht_scbelow) = ttf.getsize('ç')
    (wd_yhead, ht_yhead) = ttf.getsize('ý')
    (wd_cCbelow, ht_cCbelow) = ttf.getsize('Ç')
    
    return ({'gpqy': ht_o - ht_g, 
             'j': ht_i - ht_j,
             'Q@&$': ht_O - ht_Q,
             ',': ht_dot - ht_comma,
             ';': ht_colon - ht_semicolon,
             '|()_': ht_l - ht_or,
             'çęŋųƍƹȝȷȿɿʅ': ht_o - ht_scbelow,
             'ýÿĝğġģįĵţÿĝğġģįĵţ': ht_l - ht_yhead, 
...
def _getKeyVal(char, desasc_dict):
    ...
    """
    val = 0
    for key in list(desasc_dict.keys()):
        if key.find(char) != -1:
            val = desasc_dict[key]
            break            
    return (val)        
...
def Praster(direct, fontpath, langType, codeMethod='utf_8', 
            text=['The quick brown fox jumps over the lazy dog.', 'The lazy tabby cat sleeps in the sun all afternoon.'],
            dim=(1024,768), fg=(0,0,0), bg=(232,232,232), wfont=None, regfile=True, lmargin=86, tmargin=86, 
    ...
    if fwd: # Reset fht if fwd is specified. This isn't working quite like I'd like. See full set of font species for details.
        ...
        std_char = 'n' # u'n' is used as a standard character for calculating font size 
...
def Gen_Bitmap_RegFile(direct, fontName, langType, textFileNameList, genmethod=2, codeMethod='utf_8', dim=(1024,768), fg=(0,0,0), bg=(232,232,232), 
    lmargin=198, tmargin=55, linespace=50, fht=14, fwd=None, bbox=False, bbox_big=False, addspace=18, log=False):
    """
    ...
    arguments:
     ...
     the following arguments are identical to Praster    
        ...
        fg=(0,0,0)      : RGB font color
        bg=(232,232,232): RGB background color
        lmargin=86      : left margin in pixels (previous lmargin=215 for 1280x1024)
        tmargin=86      : top margin in pixels NOTE ORIGIN IS IN BOTTOM LEFT CORNER
        linespace=43    : linespacing in pixels (previous linespace=65 for 1280x1024)
        fht=18          : font height in pixels (max vertical distance between highest and lowest painted pixel
                        : considering every character in the font). Makes more sense to specify _width_,
                        : but ImageFont.truetype() wants a ht). Not every font obeys; see, e.g., "BrowalliaUPC Regular"
    ...
    if langType in EngLangList:
        ...
        fd = FontDict(); fontpath = fd.fontGet(fontName,'Regular')   # fd = FontDict(); fontpath = fd.fontGet(fontName,'Regular')
    ...
    if genmethod == 0:
        ...
        if langType in EngLangList:
            ...
            Praster(direct, fontpath, langType, text=['This is a test.', 'This is another.'], fht=fht)
            Praster(direct, fontpath, langType, text=['This is a one-liner.'], fht=fht)
        elif langType in ChnLangList:
            Praster(direct, fontpath, langType, text=['我们|爱|你。', '为什么|不让|他|走？'], fht=fht)
        elif langType == 'Korean':
            Praster(direct, fontpath, langType, text=['저는 7년 동안 한국에서 살았어요', '이름은 무엇입니까?'], fht=fht)
        elif langType == 'Japanese':
            Praster(direct, fontpath, langType, text=['むかし、 むかし、 ある ところ に', 'おじいさん と おばあさん が いました。'], fht=fht)
        else:
            raise ValueError("invalid langType %s!" % langType)
        ...
        if not os.path.isfile(realtxtfile):
            ...
            lines[0] = re.sub("\ufeff", "", lines[0]) # remove file starter '\ufeff'     
            ...
            tmp2[len(tmp2)-1] = re.sub("\r\n$", "", tmp2[len(tmp2)-1])    # remove "\r\n" at the ending of the last line
...
def updReg(direct, regfileNameList, addspace):
    ...
    for trialID in range(len(regfileNameList)):
        ...
        # add WordID
        RegDF['WordID'] = list(range(1,len(RegDF)+1))        
        # add line        
        RegDF['line_no'] = 0
...
def cal_crlSacFix(direct, subj, regfileNameList, ExpType, recStatus=True, diff_ratio=0.6, frontrange_ratio=0.2, y_range=60, fix_method='DIFF', addCharSp=1):
    ...
    if datafileExist and regfileExist:
        ...
        for trialID in np.unique(list(map(int,SacDF.trial_id))):
            RegDF = _getRegDF(direct, regfileNameList, trialID)  # get region file
...
def _image_SacFix(direct, subj, bitmapNameList, Sac, crlSac, Fix, crlFix, RegDF, trialID, method, max_FixRadius, drawFinal, showNum, PNGmethod):
    ...
    if PNGmethod == 0:
        ...
        fg = (0,0,0); bg = (232,232,232); dim = (1024,768)
        # initialize images
        ...
        for curline in pd.unique(RegDF.line_no):
            line = RegDF[RegDF.line_no==curline]; line.index = list(range(len(line)))            
